...
app.get("/watchlist/home", async (req, res) => {
	try {
		...
		// TRENDING MOVIES – Top 8 rated series
		...
		// Fetch distinct genres for your home page
...
`);
		...
		// Fetch series for each genre (max 8 per genre)
  ...
  `,
		...
		res.render("pages/home", {
			heroSeries,
			trendingMovies,
			genreSeriesRows, // ✅ Pass genre-wise rows to EJS
...
app.get("/watchlist/series/:id", (req, res) => {
	...
	const q1 = `
    SELECT s.series_id, s.title, s.release_year, s.summary, s.platform, s.landscape_poster_url,
